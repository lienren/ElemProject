                ...
                <el-row>
                  <el-col :span="10">
                    <div style="width:65px;">
                      <img
                        :src="infoBusinessUser.busHeadImg"
...
export default {
  ...
  methods: {
    init () {
      this.getPlayPCDetail(this.id)
    },
    async getPlayPCDetail (pcid) {
      if (!pcid || pcid === 0 || pcid === '0') {
        return
      }
      const result = await api.getPlayParentChildDetail({
        id: pcid
      })
